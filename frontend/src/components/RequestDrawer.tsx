...
export default function RequestDrawer({ requestId, onClose, onRated }: Props) {
  ...
  return <dialog ref={dialog} className={styles.drawer} aria-labelledby="request-detail-title" onCancel={(event) => { event.preventDefault(); onClose() }} onClick={(event) => {
    ...
    <div className={styles.body}>
      ...
      {record && <>
        {record.status === 'failed' && <div className={ui.error}>Request failed: {record.error_code || 'provider error'}</div>}
        ...
        <section className={ui.section}><h3 className={ui.sectionTitle}>Cost breakdown</h3><dl className={styles.facts}>
          <div><dt>Actual estimate</dt><dd>{formatCost(record.actual_cost_usd)}</dd></div><div><dt>Reference</dt><dd>{record.status === 'failed' ? 'N/A' : formatCost(record.reference_cost_usd)}</dd></div><div><dt>Saved</dt><dd>{record.status === 'failed' ? 'N/A' : formatCost(record.actual_cost_usd === null ? null : record.reference_cost_usd - record.actual_cost_usd)}</dd></div><div><dt>Input / output tokens</dt><dd>{record.prompt_tokens} / {record.completion_tokens}</dd></div>
        </dl></section>
        {!!record.attempts?.length && <section className={ui.section}><h3 className={ui.sectionTitle}>Provider attempts</h3>{record.attempts.map((attempt) => <details key={attempt.id} className={styles.attempt}><summary>{attempt.sequence}. {attempt.kind === 'self_check' ? 'Self-check' : 'Answer'} / {attempt.model} / {attempt.status}</summary><dl className={styles.facts}><div><dt>Input / output tokens</dt><dd>{attempt.usage_details.usage_known === false ? 'Unknown' : `${attempt.prompt_tokens} / ${attempt.completion_tokens}`}</dd></div><div><dt>Estimated cost</dt><dd>{formatCost(attempt.cost_usd)}</dd></div><div><dt>Latency</dt><dd>{formatLatency(attempt.latency_ms)}</dd></div><div><dt>Error</dt><dd>{attempt.error_code || 'None'}</dd></div></dl><pre className={styles.content}>{JSON.stringify(attempt.usage_details, null, 2)}</pre></details>)}</section>}
        ...
        {record.status !== 'failed' && <section className={ui.section}><h3 className={ui.sectionTitle}>Feedback</h3><form onSubmit={(event) => { event.preventDefault(); void saveFeedback() }}>
