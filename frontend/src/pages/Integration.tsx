// Show workspace readiness and source-only SDK examples without copying or retaining credentials.
...
export default function Integration({ modelsReady, onRefresh }: Props) {
  ...
  const install = `git clone https://github.com/umarsayed12/SmartRoute.git\ncd SmartRoute\n${shell === 'powershell' ? 'python' : 'python3'} -m venv .venv\n${shell === 'powershell' ? '.\\.venv\\Scripts\\python.exe' : '.venv/bin/python'} -m pip install ./sdk`
  ...
  return <div className={ui.page}>
    ...
    <section className={ui.section}>
      <div className={ui.toolbar}><h3 className={ui.sectionTitle}>Source install <span className={styles.badge}>Not published to PyPI</span></h3><fieldset className={styles.switches} aria-label="Command shell">{(['powershell', 'posix'] as const).map((value) => <label key={value}><input type="radio" name="command-shell" checked={shell === value} onChange={() => { setShell(value); setCopied('') }} />{value === 'powershell' ? 'PowerShell' : 'Bash'}</label>)}</fieldset></div>
      <div className={styles.codeHeading}><span>Python 3.11+</span><button type="button" className={ui.iconButton} title="Copy source installation" aria-label="Copy source installation" onClick={() => void copy(install, 'Source installation')}><Copy size={14} /></button></div><pre className={styles.code}><code>{install}</code></pre>
    </section>
    <section className={ui.section}>
    ...
    <div className={`${ui.actions} ${styles.links}`}><a className={ui.button} href="https://github.com/umarsayed12/SmartRoute/tree/main/sdk" target="_blank" rel="noreferrer">SDK reference<ArrowUpRight size={14} /></a><Link to="/account" className={ui.button}>Manage gateway keys</Link></div>
  </div>
}