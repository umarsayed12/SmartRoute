...
export default function Settings() {
  ...
  return <div className={ui.page}>
    ...
    <div className={styles.configuration}>
      ...
      <section className={ui.section}><h2 className={ui.sectionTitle}>Health</h2><dl className={styles.health}>{checks.map((check) => <div key={check.name}><dt>{check.name}</dt><dd className={check.state === null ? ui.muted : check.state ? ui.good : ui.bad}>{check.state === null ? 'Unknown' : <>{check.state ? <Check size={14} /> : <X size={14} />}{check.state ? check.yes : check.no}</>}</dd></div>)}</dl></section>
    </div>
